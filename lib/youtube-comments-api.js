...
const YouTubeCommentsAPI = {
    ...
    try {
      const results = await chrome.scripting.executeScript({
        ...
        func: () => {
          ...
          // Like count from DOM
          let likeCount = 0;
          // Try segmented like button (new layout)
          const likeBtn = document.querySelector('like-button-view-model button, ytd-toggle-button-renderer #button[aria-label]');
          if (likeBtn) {
            const ariaLabel = likeBtn.getAttribute('aria-label') || '';
            const likeMatch = ariaLabel.match(/([\d.,\s]+)/);
            if (likeMatch) {
              likeCount = parseInt(likeMatch[1].replace(/[\s.,]/g, ''), 10) || 0;
            }
          }

          // Comment count from comments header
          let commentCount = 0;
          const commentsHeader = document.querySelector('ytd-comments-header-renderer #count .count-text, ytd-comments-header-renderer #count');
          if (commentsHeader) {
            const text = commentsHeader.textContent || '';
            const countMatch = text.match(/([\d.,\s]+)/);
            if (countMatch) {
              commentCount = parseInt(countMatch[1].replace(/[\s.,]/g, ''), 10) || 0;
            }
          }
  ...
  // Recursive search through nested YouTube JSON
  _searchDict(obj, key) {
    const results = [];
    const stack = [obj];
    while (stack.length) {
